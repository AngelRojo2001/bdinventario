...
$libro = mysql_query("
    SELECT i.nro, i.fecha, i.autor, i.titulo, u.nombre lugar, e.nombre editorial,
        i.anio, i.edicion, i.volumen, i.paginas, i.compra, i.precio, i.donac, i.dl,
        p.nombre procedencia, i.observacion
    FROM libro i
    JOIN lugar u ON i.id_lug = u.id
    JOIN editorial e ON i.id_edi = e.id
    JOIN procedencia p ON i.id_pro = p.id
    ORDER BY nro ASC", $conex);

$i = 1;
$objPHPExcel->getActiveSheetIndex();
while ($row_libro = mysql_fetch_array($libro)) {
    $objPHPExcel->getActiveSheet()->getStyle('A'.$i.':M'.$i)->getFont()->setBold(TRUE);
    $objPHPExcel->getActiveSheet()->getStyle('A'.$i.':M'.$i)->getAlignment()
            ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    
    $objPHPExcel->getActiveSheet()->getStyle('A'.($i+2).':M'.($i+2))->getFont()->setBold(TRUE);
    $objPHPExcel->getActiveSheet()->getStyle('A'.($i+2).':M'.($i+2))->getAlignment()
            ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    
    $objPHPExcel->getActiveSheet()->mergeCells('C'.$i.':E'.$i);
    $objPHPExcel->getActiveSheet()->mergeCells('C'.($i+1).':E'.($i+1));
    $objPHPExcel->getActiveSheet()->mergeCells('F'.$i.':I'.$i);
    $objPHPExcel->getActiveSheet()->mergeCells('F'.($i+1).':I'.($i+1));
    $objPHPExcel->getActiveSheet()->mergeCells('J'.$i.':K'.$i);
    $objPHPExcel->getActiveSheet()->mergeCells('J'.($i+1).':K'.($i+1));
    $objPHPExcel->getActiveSheet()->mergeCells('L'.$i.':M'.$i);
    $objPHPExcel->getActiveSheet()->mergeCells('L'.($i+1).':M'.($i+1));
    
    $objPHPExcel->getActiveSheet()->mergeCells('I'.($i+2).':J'.($i+2));
    $objPHPExcel->getActiveSheet()->mergeCells('I'.($i+3).':J'.($i+3));
    $objPHPExcel->getActiveSheet()->mergeCells('K'.($i+2).':M'.($i+2));
    $objPHPExcel->getActiveSheet()->mergeCells('K'.($i+3).':M'.($i+3));
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A'.$i.':M'.($i+3))
            ->applyFromArray($styleArray);
    
    $objPHPExcel->getActiveSheet()->setCellValue('A'.$i, "Nº INV");
    $objPHPExcel->getActiveSheet()->setCellValue('A'.($i+1), $row_libro['nro']);
    $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(10);
    $objPHPExcel->getActiveSheet()->setCellValue('B'.$i, "FECHA");
    $objPHPExcel->getActiveSheet()->setCellValue('B'.($i+1), $row_libro['fecha']);
    $objPHPExcel->getActiveSheet()->setCellValue('C'.$i, "AUTOR");
    $objPHPExcel->getActiveSheet()->setCellValue('C'.($i+1), $row_libro['autor']);
    $objPHPExcel->getActiveSheet()->setCellValue('F'.$i, "TÍTULO");
    $objPHPExcel->getActiveSheet()->setCellValue('F'.($i+1), recortar($row_libro['titulo'], 20));
    $objPHPExcel->getActiveSheet()->setCellValue('J'.$i, "LUGAR");
    $objPHPExcel->getActiveSheet()->setCellValue('J'.($i+1), $row_libro['lugar']);
    $objPHPExcel->getActiveSheet()->setCellValue('L'.$i, "EDITORIAL");
    $objPHPExcel->getActiveSheet()->setCellValue('L'.($i+1), $row_libro['editorial']);
    
    
    $objPHPExcel->getActiveSheet()->setCellValue('A'.($i+2), "AÑO");
    $objPHPExcel->getActiveSheet()->setCellValue('A'.($i+3), $row_libro['anio']);
    $objPHPExcel->getActiveSheet()->setCellValue('B'.($i+2), "ED.");
    $objPHPExcel->getActiveSheet()->setCellValue('B'.($i+3), $row_libro['edicion']);
    $objPHPExcel->getActiveSheet()->setCellValue('C'.($i+2), "VOL.");
    $objPHPExcel->getActiveSheet()->setCellValue('C'.($i+3), $row_libro['volumen']);
    $objPHPExcel->getActiveSheet()->setCellValue('D'.($i+2), "PAGS.");
    $objPHPExcel->getActiveSheet()->setCellValue('D'.($i+3), $row_libro['paginas']);
    $objPHPExcel->getActiveSheet()->setCellValue('E'.($i+2), "COMP.");
    $objPHPExcel->getActiveSheet()->setCellValue('E'.($i+3), $row_libro['compra']);
    $objPHPExcel->getActiveSheet()->setCellValue('F'.($i+2), "PRECIO");
    $objPHPExcel->getActiveSheet()->setCellValue('F'.($i+3), $row_libro['precio']);
    $objPHPExcel->getActiveSheet()->setCellValue('G'.($i+2), "DONAC.");
    $objPHPExcel->getActiveSheet()->setCellValue('G'.($i+3), $row_libro['donac']);
    $objPHPExcel->getActiveSheet()->setCellValue('H'.($i+2), "D.L.");
    $objPHPExcel->getActiveSheet()->setCellValue('H'.($i+3), $row_libro['dl']);
    $objPHPExcel->getActiveSheet()->setCellValue('I'.($i+2), "PROCEDENCIA");
    $objPHPExcel->getActiveSheet()->setCellValue('I'.($i+3), $row_libro['procedencia']);
    $objPHPExcel->getActiveSheet()->setCellValue('K'.($i+2), "OBSERVACIONES");
    $objPHPExcel->getActiveSheet()->setCellValue('K'.($i+3), recortar($row_libro['observacion'], 20));

    $i += 5;
}

$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':M'.($i-1))
        ->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);

$objPHPExcel->getActiveSheet()->setTitle("LIBROS");

/* Modificar desde ahi, para hacer funcionar 
...
while($row_carrera = mysql_fetch_array($carrera)){
    ...
    $objPHPExcel->getActiveSheet()->getStyle('G3:H3')->getFill()
            ->getStartColor()->setARGB('FFFF0000');
    $objPHPExcel->getActiveSheet()->setCellValue('G3',"AÃ‘O");
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('K3',"NÂº PAG");
    ...
    while($row_registro = mysql_fetch_array($registro)){
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C'.$i,recortar($row_registro['titulo'], 40));
...
function recortar($cadena, $tam) {
    ...
    $titulo_nuevo = '';
    $sum = 0;
    for ($j = 0; $j < count($titulo_es)-1; $j++) {
        $sum += strlen($titulo_es[$j]) + 1;
        if ($sum < $tam) {
            $titulo_nuevo .=  "$titulo_es[$j] ";
        }
        else {
            $titulo_nuevo .= "$titulo_es[$j]\n";
            $sum = 0;
